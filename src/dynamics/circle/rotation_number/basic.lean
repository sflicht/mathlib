...
@[trans] lemma ordering.compares.trans {α} [preorder α] {x y z : α} :
  ∀ {o : ordering}, o.compares x y → o.compares y z → o.compares x z
| ordering.lt := lt_trans
| ordering.eq := eq.trans
| ordering.gt := gt_trans

def ordering.compares_or_eq {α} [has_lt α] (o : ordering) (x y : α) := o.compares x y ∨ x = y

lemma ordering.compares.compares_or_eq {α} [has_lt α] {o : ordering} {x y : α} :
  o.compares x y → o.compares_or_eq x y :=
or.inl

@[trans] lemma ordering.compares.trans_or_eq {α} [preorder α] {x y z : α} {o : ordering}
  (hxy : o.compares x y) (hyz : o.compares_or_eq y z) :
  o.compares x z :=
hyz.elim hxy.trans (λ h, trans_rel_left _ hxy h)

@[trans] lemma ordering.compares_or_eq.trans_compares {α} [preorder α] {x y z : α} {o : ordering}
  (hxy : o.compares_or_eq x y) (hyz : o.compares y z) :
  o.compares x z :=
hxy.elim (λ h, h.trans hyz) (λ h, trans_rel_right _ h hyz)

@[trans] lemma ordering.compares_or_eq.trans {α} [preorder α] {x y z : α} {o : ordering}
  (hxy : o.compares_or_eq x y) (hyz : o.compares_or_eq y z) :
  o.compares_or_eq x z :=
hxy.elim (λ h, (h.trans_or_eq hyz).compares_or_eq) (λ h, trans_rel_right _ h hyz)

lemma monotone.order_dual {α β} [preorder α] [preorder β] {f : α → β} (hf : monotone f) :
  @monotone (order_dual α) (order_dual β) _ _ f :=
λ x y h, hf h

lemma strict_mono.order_dual {α β} [has_lt α] [has_lt β] {f : α → β} (hf : strict_mono f) :
...
lemma iterate_right (h : conjugates f ga gb) (n : ℕ) : conjugates f (ga^[n]) (gb^[n]) :=
nat.rec_on n id_right $ λ n ihn, ihn.comp_right h

lemma iterate_left {f : α → α} {g : ℕ → α → α} (H : ∀ n, conjugates f (g n) (g $ n + 1)) (n k : ℕ) :
  conjugates (f^[n]) (g k) (g $ n + k) :=
begin
  induction n with n ihn generalizing k,
  { rw [zero_add], exact id_left },
  { convert (H k).comp_left (ihn (k + 1)) using 2,
    rw [nat.succ_eq_add_one, add_right_comm, add_assoc] }
end

lemma maps_to_fixed_points (h : conjugates f ga gb) :
  set.maps_to f (fixed_points ga) (fixed_points gb) :=
assume x hx,
calc gb (f x) = f (ga x) : (h.eq x).symm
          ... = f x      : congr_arg f hx
...
@[refl] lemma refl : commute f f := λ _, eq.refl _
...
by rw [nat.iterate_succ, hx, (h.iterate_left n).eq, ihn, nat.iterate_succ']

lemma iterate_le_of_map_le [preorder α] (h : commute f g) (hf : monotone f)  (hg : monotone g)
  {x} (hx : f x ≤ g x) (n : ℕ) :
  f^[n] x ≤ (g^[n]) x :=
begin
  induction n with n ihn, { refl },
  rw [nat.iterate_succ, nat.iterate_succ'],
  apply le_trans (hf.iterate n hx),
  rw [(h.iterate_left n).eq],
  exact hg ihn
end

lemma iterate_succ_lt_of_map_lt [preorder α] (h : commute f g) (hf : monotone f)
  (hg : strict_mono g) {x} (hx : f x < g x) (n : ℕ) :
  f^[n+1] x < (g^[n+1]) x :=
begin
  induction n with n ihn, { exact hx },
  rw [nat.iterate_succ, nat.iterate_succ' g],
  apply lt_of_le_of_lt (hf.iterate (n + 1) (le_of_lt hx)),
  rw [(h.iterate_left _).eq],
  exact hg ihn
end

lemma iterate_succ_lt_of_map_lt' [preorder α] (h : commute f g) (hf : strict_mono f)
  (hg : monotone g) {x} (hx : f x < g x) (n : ℕ) :
  f^[n+1] x < (g^[n+1]) x :=
@iterate_succ_lt_of_map_lt (order_dual α) _ _ _ h.symm hg.order_dual hf.order_dual x hx n

lemma iterate_succ_lt_iff_map_lt [linear_order α] (h : commute f g) (hf : monotone f)
  (hg : strict_mono g) {x} (n : ℕ) :
  f^[n+1] x < (g^[n+1]) x ↔ f x < g x :=
begin
  rcases lt_trichotomy (f x) (g x) with H|H|H,
  { simp only [*, iterate_succ_lt_of_map_lt] },
  { simp only [*, h.iterate_eq_of_map_eq, lt_irrefl] },
  { simp only [lt_asymm H, lt_asymm (h.symm.iterate_succ_lt_of_map_lt' hg hf H n)] }
end

lemma iterate_succ_lt_iff_map_lt' [linear_order α] (h : commute f g) (hf : strict_mono f)
  (hg : monotone g) {x} (n : ℕ) :
  f^[n+1] x < (g^[n+1]) x ↔ f x < g x :=
@iterate_succ_lt_iff_map_lt (order_dual α) _ _ _ h.symm hg.order_dual hf.order_dual x n
...
lemma compares_impl_of_semiconj_by_of_mono {α : 
