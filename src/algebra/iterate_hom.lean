...
lemma mul_left_iterate [monoid M] (a : M) (n : ℕ) : ((*) a^[n]) = (*) (a^n) :=
nat.rec_on n (funext $ λ x, by simp) $ λ n ihn,
funext $ λ x, by simp [iterate_succ, ihn, pow_succ', mul_assoc]

lemma add_left_iterate [add_monoid M] (a : M) (n : ℕ) : ((+) a^[n]) = (+) (n •ℕ a) :=
@mul_left_iterate (multiplicative M) _ a n

lemma mul_right_iterate [monoid M] (a : M) (n : ℕ) : ((λ x, x * a)^[n]) = (λ x, x * a^n) :=
nat.rec_on n (funext $ λ x, by simp) $ λ n ihn,
funext $ λ x, by simp [iterate_succ, ihn, pow_succ, mul_assoc]

lemma add_right_iterate [add_monoid M] (a : M) (n : ℕ) : ((λ x, x + a)^[n]) = λ x, x + (n •ℕ a) :=
